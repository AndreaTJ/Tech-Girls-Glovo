...
resultado_clase_tipo <- obtener_clase_tipo(airbnb_data)
...
Convertimos algunas columnas del conjunto de datos en variables categóricos (factores) ya que esta transformación, nos va a permitir tratar las variables  `Room.type, Neighbourhood, City y Country`, como categorías discretas en vez de datos continuos. Facilitando así el análisis y la visualización.
...
Con la información de arriba, comprobamos que efectivamente `Data.last.review y Update.Date` son ahora fechas y que `Room.type, Neighbourhood, City y Country` son ahora variables catégoricas.

Echamos un vistazo, al resultado de la categorización.

```{r}
# Identificar las variables categóricas (factores )
vars_categoricas <- sapply(airbnb_data, function(x) is.factor(x))

# Aplicar summary() solo a las variables categóricas
resumen_categoricas <- summary(airbnb_data[vars_categoricas])
...
Miramos que registros son únicos de España:
...
# Filtrar para mantener solo las filas donde Country es "Spain"
datos_espana <- airbnb_data |> 
  filter(Country == "Spain")

# Contar el número de filas para cada City en España
conteo_ciudades <- datos_espana |> 
  group_by(City) |>  
  summarise(Count = n())
...
# Verificar qué dato en la columna City tenemos en el  dataset original
print(as.data.frame(table(airbnb_data$City)))
...
Comparamos la columna `Location` con la combinación de `Country, City, y Neighbourhood`, y pudimos concluir que era redundante por lo que se procedió a su eliminación.
...
Comparamos la cuenta de los registros donde el número de reseñas es cero, con el número de cuántos de registros que no solo están sin reseñas, sino que también carecen de fecha de última reseña y de promedio de reseñas por mes.

Este paso nos permite comprobar si nuestra teoría sobre la relación entre estas variables se sostiene. Tiene sentido, ya que si un alojamiento no ha recibido reseñas, no podría tener una fecha de última reseña ni un promedio de reseñas mensuales.
...
Ahora voy a categorizar `Number.of.reviews` para un mejor análisis
...
Haremos lo mismo con `Number.of.reviews.per.month`
...
Resumen más condensado de los cambios realizados en cada columna de tu conjunto de datos:

#### 1. **Date.last.review & Updated.Date**

-   Convertidas a formato de fecha (`%Y-%m-%d`).

-   Se ha hecho una Resta entre `Fecha del Scrapping(2020-08-07) y Date.last.review.` El resultado se guarda en *`Time_category`* y se categorizada en rangos de tiempo:
...
-   Categorizada en rangos desde 'No disponible' hasta '181 a 365 días'. Así:
    ...
    -   "No disponible" - Para los alojamientos con cero días disponibles.
    -   "Menos de 30 días"
    -   "31 a 60 días"
    -   "61 a 90 días"
    -   "91 a 180 días"
...
airbnb_data1 <- select(airbnb_data, -Name)
