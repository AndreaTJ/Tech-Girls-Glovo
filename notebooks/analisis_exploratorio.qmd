...
#Revisa cuántos y qué porcentaje de las filas tienen coincidencia exacta entre Location y la columna Combined

table(airbnb_data$Location_Match) #Cuenta solo lo que es TRUE

```

Comparamos la columna `Location` con la combinación de `Country, City, y Neighbourhood` y podemos ver que son iguales.`Location` es, por tanto, redundante; por lo que se puede eliminar.
...
Eliminamos el registro con `Room.Price` de cero porque probablemente es un error de entrada, ya que es poco probable que una habitación de hotel tenga un coste cero. Este error podría distorsionar nuestro análisis estadístico y posterior modelo predictivo. Eliminando este dato, aseguramos la calidad y consistencia del dataset.
...
`*La fecha de extracción, ha sido obtenida directamente del apartado de "Information" del dataset: Information Airbnb Listings en OpenDataSoft.`
...
numeric_data <- airbnb_data %>% 
  select(Room.ID, Host.ID, Room.Price, Minimum.nights, Number.of.reviews, Number.of.reviews.per.month, Rooms.rent.by.the.host)
...
Al analizar los outliers de los atributos numéricos, percibimos que aquellos que tienen **datos extraños son el precio por habitación, el mínimo de noches, el número de reviews, el número de reviews por mes y las habitaciones alquiladas por cada host.** Estos datos se normalizarán en el paso siguiente de pre-procesamiento. Por otra parte, **el Room ID y el Host ID no presentan outliers.**
...
**`¿Cuáles son los distritos más populares en Airbnb?`**
...
### Top 10 - Tipo de propiedad y áreas (distritos)

```{r}
library(dplyr)
library(ggplot2)

# Calcula la cantidad total de listados por distrito
distrito_counts <- airbnb_data %>%
  group_by(Neighbourhood) %>%
  summarise(Total = n()) %>%
  arrange(desc(Total))

# Selecciona los top 10 distritos
top_distritos <- distrito_counts %>%
  top_n(10, Total) %>%
  pull(Neighbourhood)

# Filtra el dataset original para incluir solo esos distritos
filtered_data <- airbnb_data %>%
  filter(Neighbourhood %in% top_distritos)

# Crear el gráfico de barras apiladas para los top 10 distritos
ggplot(data = filtered_data, aes(x = Neighbourhood, fill = `Room.type`)) +
  geom_bar() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(x = "Distrito", y = "Cantidad") +
  ggtitle("Distribución de tipos de propiedad por distritos en Madrid")
# 
```

**`¿Cómo se distribuyen los diferentes tipos de habitaciones en el top 10 de distritos de Madrid y qué distritos presentan una mayor diversidad de opciones de alojamiento?`**

**Diversidad de Habitaciones:**

-   **Embajadores y Universidad**: Son los distritos con más alojamientos y una amplia variedad de tipos de habitaciones, predominando los apartamentos enteros y habitaciones privadas, con escasas opciones de habitaciones compartidas y de hotel.
...
-   **Menor Diversidad**: Distritos como Tetuán, Triángulo del Arte, Chamberí, y Puerta del Ángel tienen menos opciones y se enfocan más en apartamentos enteros y habitaciones privadas, con limitadas ofertas de habitaciones de hotel o compartidas.

### **`Top 10 - Distribución de precios vs distrito`** 
...
# Crea el gráfico de boxplot para los top 10 distritos
ggplot(data = filtered_data, aes(x = Neighbourhood, y = Room.Price)) +
  geom_boxplot(outlier.colour = "red", outlier.shape = 1) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(x = "Distrito", y = "Precio (€)", title = "Variabilidad de precios por distrito en Madrid - Top 10 Distritos") +
  theme_minimal()


```

**`¿Cómo varían los precios de los alojamientos de Airbnb entre diferentes distritos del top10 de Madrid y cuáles presentan la mayor variabilidad en sus precios?`**

-   **Distritos con Mayor Variabilidad**: Distritos como Embajadores, Universidad, y Palacio muestran una amplia variabilidad en precios, con numerosos valores atípicos que indican precios extremadamente altos comparados con la mediana.

-   **Distritos con Menor Variabilidad**: Distritos como Justicia, Cortes, y Puerta del Ángel parecen tener una variabilidad más contenida, con menos valores atípicos y cajas más estrechas, lo que sugiere precios más consistentes y predecibles. 
...
-   **Correlaciones Notables**:
    ...
    -   **Rooms.rent.by.the.host vs. Room.Price**: Presenta una correlación positiva moderada (0.170\*\*\*), sugiriendo que los anfitriones que rentan más habitaciones tienden a cobrar precios más altos, aunque la relación no es muy fuerte.
...
1.  **Distritos más populares en Airbnb**

2.  **Correlación entre precio y número de reseñas**

3.  **Variación de precios por tipo de habitación**

4.  **Distribución de tipos de habitación por distrito**

5.  **Variabilidad de precios por distrito**

6.  **Distribución geográfica de listados**

7.  **Correlaciones principales con el precio**
